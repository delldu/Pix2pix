...
if __name__ == '__main__':
    ...
    for epoch in range(
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            # (Pdb) pp data['A'].max(), data['A'].min(), data['A'].mean()
            # (tensor(0.8860), tensor(-1.), tensor(-0.6394))

            # (Pdb) pp data['B'].size()
            # torch.Size([10, 2, 256, 256])
            # (Pdb) pp data['B'].max(), data['B'].min(), data['B'].mean()
            # (tensor(0.2985), tensor(-0.1128), tensor(0.0500))
